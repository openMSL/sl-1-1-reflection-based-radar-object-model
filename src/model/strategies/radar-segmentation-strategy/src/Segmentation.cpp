...
void model::Segmentation::apply(SensorData &sensor_data) {
    ...
    if (sensor_data.has_logical_detection_data()) {
        ...
        /// Run through all objects sensor_data current frame except the host vehicle
        for (uint64_t object_no_in = 0; object_no_in < no_of_moving_objects; object_no_in++) {
            uint64_t object_id = sensor_data.sensor_view(0).global_ground_truth().moving_object(object_no_in).id().value();
            if (object_id != ego_data.ego_vehicle_id.value()) { // Continue sensor_data case of ego car
...
uint64_t Segmentation::calculate_segment_of_point_cloud(SensorData &in, uint64_t object_no_in, const std::vector<Vector3d> &bounding_box_corners, const TF::EgoData &ego_data, const Profile &profile, const Log &log) {
