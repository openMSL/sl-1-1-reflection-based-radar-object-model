...
void PointcloudFusion::apply(SensorData& sensor_data)
{
    ...
    if (!sensor_data.sensor_view(0).lidar_sensor_view().empty())
    {
        ...
        {
            calculate_fused_pointcloud_for_given_lidar_sensors(sensor_data);
    ...
    else if (!sensor_data.sensor_view(0).radar_sensor_view().empty())
    {
        ...
        {
            calculate_fused_pointcloud_for_given_radar_sensors(sensor_data);
            log("Size of logical detections after point cloud fusion: " + std::to_string(sensor_data.logical_detection_data().logical_detection_size()));
        }
        else
        {
...
//// Functions
void PointcloudFusion::calculate_fused_pointcloud_for_given_lidar_sensors(SensorData& sensor_data)
{
...
void PointcloudFusion::calculate_fused_pointcloud_for_given_radar_sensors(SensorData& sensor_data)
{
    sensor_data.mutable_logical_detection_data()->clear_logical_detection();

    for (int sensor_idx = 0; sensor_idx < sensor_data.feature_data().radar_sensor_size(); sensor_idx++)
    {
        for (int detection_no = 0; detection_no < sensor_data.feature_data().radar_sensor(sensor_idx).detection_size(); detection_no++)
        {
