...
void RosReflections::apply(SensorData& sensor_data)
{
...
RosReflections::RosReflections(const Profile& profile, const Log& log, const Alert& alert) : model::Strategy(profile, log, alert)
{
...
reflections::WorkerPCL::WorkerPCL(const std::string& topic, std::string frame_id)
...
void reflections::WorkerPCL::injectRadar(SensorData& sensor_data, int sensor_no, const Log& log)
{
    ...
    if (no_of_rendering_results == 0)
    {
        ...
        log("No reflection from sensor " + std::to_string(sensor_no) + " for ROS output at timestamp " + std::to_string(timestamp));
        return;
    }
    ...
    {
        auto signal_strength_in_dB = radar_sensor_view.reflection(reflection_idx).signal_strength();
