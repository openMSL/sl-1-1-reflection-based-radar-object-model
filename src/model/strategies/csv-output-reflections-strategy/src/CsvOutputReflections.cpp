        ...
        if (no_of_radar_sensors > 0) {
            ...
            for (auto & radar_sensor_view : sensor_view.radar_sensor_view()) {
                ...
                for (auto & reflection : radar_sensor_view.reflection()) {
                    ...
                    write_data_to_CSV(file_path_reflections, timestamp, radar_reflection_idx, azimuth_rad * 180 / M_PI,
                                      elevation_rad * 180 / M_PI, distance, signal_strength_in_dBm);
...
void CsvOutputReflections::write_data_to_CSV(const std::string& path, double timestamp, size_t reflection_idx, double azimuth_in_deg, double elevation_in_deg, double distance, double signal_strength) {
    std::fstream my_file;
    my_file.open(path, std::ios::app);
