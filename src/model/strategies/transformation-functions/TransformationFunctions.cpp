...
Vector3d TransformationFunctions::transform_position_from_sensor_to_object_coordinates(const Vector3d &pcl_segment_position_in_sensor, const TransformationFunctions::EgoData &ego_data, const MountingPosition &mounting_pose, const BaseMoving &object_base) {
    Vector3d pcl_segment_position_in_ego = TransformationFunctions::transform_from_local_coordinates(pcl_segment_position_in_sensor, mounting_pose.orientation(), mounting_pose.position());
    Vector3d position_in_world_coordinates = TransformationFunctions::transform_position_from_ego_to_world_coordinates(pcl_segment_position_in_ego, ego_data);
    Vector3d position_in_object_coordinates = TransformationFunctions::transform_to_local_coordinates(position_in_world_coordinates, object_base.orientation(), object_base.position());
    return position_in_object_coordinates;
}

Vector3d TransformationFunctions::transform_position_from_object_to_sensor_coordinates(const Vector3d &pcl_segment_position_in_object, const TransformationFunctions::EgoData &ego_data, const MountingPosition &mounting_pose, const BaseMoving &object_base) {
    Vector3d position_in_world_coordinates = TransformationFunctions::transform_from_local_coordinates(pcl_segment_position_in_object, object_base.orientation(), object_base.position());
    Vector3d position_in_ego_coordinates = TransformationFunctions::transform_position_from_world_to_ego_coordinates(position_in_world_coordinates, ego_data);
    Vector3d position_in_sensor_coordinates = TransformationFunctions::transform_to_local_coordinates(position_in_ego_coordinates, mounting_pose.orientation(), mounting_pose.position());
